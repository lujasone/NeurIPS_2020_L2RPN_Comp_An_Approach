...
class MyCloseToOverFlowNoFlowReward(BaseReward):
    """
    ...
    def __call__(self, action, env, has_error,
        ...
        if has_error or is_illegal or is_ambiguous:
            ...
            # penalty on no_rho_while_connected
            line_stat_s = env.current_obs.line_status
            lineflow_ratio = env.current_obs.rho
            no_rho_while_connected = line_stat_s & (lineflow_ratio == 0)
            if np.any(no_rho_while_connected):
                no_rho_pen = np.count_nonzero(no_rho_while_connected)
                close_to_overflow += dt_float(no_rho_pen / 2)
            ...
            if len(cascade_failure_list) > 0:
                close_to_overflow += dt_float(len(cascade_failure_list) / 2)

            close_to_overflow = np.clip(close_to_overflow,
                                        dt_float(0.0), self.max_overflowed)
            res = np.interp(close_to_overflow,
                            [dt_float(0.0), self.max_overflowed],
...
class MyLinesCapacityReward(BaseReward):
    ...
    def __call__(self, action, env, has_error, is_done, is_illegal, is_ambiguous):
        if has_error or is_illegal or is_ambiguous:
            ...
            # usage = np.sum(obs.rho[obs.line_status == True])
            usage = np.clip(usage, 0.0, float(n_connected))
            res = np.interp(n_connected - usage,
                            [dt_float(0.0), float(n_connected)],
                            [self.reward_min, self.reward_max])
        return dt_float(res)
...
class MySandboxReward(BaseReward):
    def __init__(self, alpha_redisph=1.0):
        ...
        self.reward_min = dt_float(0)  # carefull here between min and max...
        self.reward_max = dt_float(1.0)
...
class MyLinesConnectedReward(BaseReward):
    ...
    def __init__(self):
        ...
        self.reward_min = dt_float(0.0)
        self.reward_max = dt_float(1.0)
        self.penalty_max_at_n_lines = dt_float(4.0)
...
class MyNewReward(BaseReward):
    ...
    def __call__(self, action, env, has_error, is_done, is_illegal, is_ambiguous):
        if not is_done and not has_error:
            ...
            res = cap - 50 * sum((((state_obs.rho - 0.99))[state_obs.rho > 1]))
            # penality weight on soft overflow
            res = res - 5 * sum((((state_obs.rho - 0.99) * \
